...
const QuotationStepSpendingLimit = () => {
    const { register, getValues, setValue, trigger, handleSubmit, formState: { errors }, } = react_hook_form_1.useForm();
    ...
    return (<form onSubmit={handleSubmit(onSubmit)} className="flex-auto h-full flex flex-col">
      <div className="flex-auto pt-16">
        ...
        validate: {
            required: value => !!getValues("spendinglimitDontKnow") || !!value
                ? true
                : "Le budget est requis",
            min: value => !!getValues("spendinglimitDontKnow") || value > 1
                ? true
            ...
            : "border-red-300 text-red-900 placeholder-red-300 focus:outline-none focus:ring-red-500 focus:border-red-500"} disabled:opacity-50`} placeholder="Valeur en €" aria-invalid={!!errors.spendinglimit} aria-describedby="spendinglimit-error" onChange={e => {
            actions.updateForm({
              ...
              <div className="sm:col-span-2 rounded-md -space-y-px">
                <div className="relative py-4 flex">
                  <div className="flex items-center h-5">
                    <input id={`spendinglimitDontKnow`} {...register("spendinglimitDontKnow")} type="checkbox" className="bg-gris-800 h-8 w-8 text-gris-800 cursor-pointer border-brown-50 focus:ring-opacity-50 focus:ring-brown-50 checked:border-brown-50" defaultChecked={state.form.spendinglimitDontKnow} onChange={e => {
            actions.updateForm({
            ...
            trigger("spendinglimit");
              ...
              {state.form.spendinglimitDontKnow && (<div className="sm:col-span-3 mt-6">
                  ...
                  <div className="grid sm:grid-cols-3 gap-x-18 mx-auto">
                    {state.data.priceRanges.map((priceRange, index) => (<div key={priceRange.code} className="rounded-md -space-y-px">
                        <div className="relative py-4 flex">
                          <div className="flex items-center h-5">
                            <input id={`priceRangeId-option-${index}`} {...register("priceRangeId", {
                validate: {
                    required: value => !!getValues("spendinglimit") || !!value
                        ? true
                        : "Vous devez sélectionner une option",
                },
            })} type="radio" className="bg-gris-800 h-8 w-8 text-gris-800 cursor-pointer border-brown-50 focus:ring-opacity-50 focus:ring-brown-50 checked:border-brown-50" value={priceRange.code} defaultChecked={state.form.priceRangeId === priceRange.code} onChange={e => actions.updateForm({
