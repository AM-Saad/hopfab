...
const HomepageBlogPostListItem = ({ blogPost, className, theme, }) => {
    ...
    return (<article className={`${className} py-10`}>
        ...
        {featuredImage && (<gatsby_1.Link to={`/blog${blogPost.uri}`} className="row-start-1 flex flex-1 overflow-hidden mb-5 lg:mb-0">
            <gatsby_plugin_image_1.GatsbyImage image={featuredImage} alt={((_g = (_f = blogPost.portraitFeaturedImage) === null || _f === void 0 ? void 0 : _f.node) === null || _g === void 0 ? void 0 : _g.alt) || ``} className="transform duration-200 ease-in-out scale-100 hover:scale-110 journal_img_wrapper"/>
          </gatsby_1.Link>)}
      </div>
    </article>);
};
